 ...
 */
export function calculateGoalProgress(goal: Goal, milestones: GoalMilestone[] = []): number {
  switch (goal.goalType) {
    ...
    case "milestone": {
      ...
      if (totalWeight > 100) {
        logger.warn(`[calculateGoalProgress] Goal ${goal.id} has milestones with total weight > 100: ${totalWeight}`);
      }
