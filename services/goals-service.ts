...
export class GoalsService {
  ...
  async listUserGoals(userId: string): Promise<UserGoalSummary[]> {
    ...
    return withUserContext(userId, async () => {
      ...
      const allMilestones =
        goalIds.length > 0
          ? await this.dbInstance.select().from(goalMilestones).where(inArray(goalMilestones.goalId, goalIds))
          : [];
  ...
  async updateGoal(
    ...
    return withUserContext(userId, async () => {
      ...
      if (updates.deadline !== undefined)
        updateData.deadline = updates.deadline ? sql`${updates.deadline}::date` : null;
      ...
      await this.dbInstance.update(goals).set(updateData).where(eq(goals.id, goalId));
  ...
  async deleteGoal(userId: string, goalId: string): Promise<void> {
    ...
    return withUserContext(userId, async () => {
      ...
      await this.dbInstance.delete(goals).where(eq(goals.id, goalId));
    });
  }
